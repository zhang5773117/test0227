...
public class RabbitConfig {
    public static final String QUEUE_CREATEORDER = "order_queue";
    public static final String QUEUE_CREATEPAY = "pay_queue";
    public static final String EXCHANGE_TOPIC_INFORM = "exchange_topic_order";
    ...
    @Bean(QUEUE_CREATEORDER)
    public Queue createOrderQueue() {
        Queue queue = new Queue(QUEUE_CREATEORDER);
        return queue;
    }

    @Bean(QUEUE_CREATEPAY)
    public Queue createPayQueue() {
        Queue queue = new Queue(QUEUE_CREATEPAY);
        return queue;
    }

    @Bean
    public Binding bindingOrder(@Qualifier(EXCHANGE_TOPIC_INFORM) Exchange exchange, @Qualifier(QUEUE_CREATEORDER) Queue queue) {
        return BindingBuilder.bind(queue).to(exchange).with("inform.#.order.#").noargs();
    }

    @Bean
    public Binding bindingOrderPay(@Qualifier(EXCHANGE_TOPIC_INFORM) Exchange exchange, @Qualifier(QUEUE_CREATEPAY) Queue queue) {
        return BindingBuilder.bind(queue).to(exchange).with("inform.#.pay.#").noargs();
    }
